...
*   特殊能力列表:
        ...
        <1>先攻 
            此怪物会比勇士先发动攻击 "1"
        <2>连击 
            此怪物每回合会多次攻击勇士 附加参数为连击次数 如 "2&3" 为三连击
        <3>吸血 
            此怪物战斗前勇士会被吸取一定百分比的血量 附加参数为吸血的倍数 如 "3&1.5" 为吸取勇士生命的1.5倍
        <4>魔攻 
            此怪物战斗时无视勇士的防御 "4"
*/
const EnemySet = {
...
const EnemysData = function(dataType, ID){
    let enemys = eval("EnemySet." + dataType);
    ...
    if(typeof(ID) !== 'undefined'){
        ...
        while(min <= max){
            ...
            if(ID === enemys[mid][0])   return enemys[mid];
            else if(ID > enemys[mid][0])    min = mid + 1;
            else    max = mid - 1;
        }
    }
